    ...
    int i;		
     ...
     PID_Init(&chassis_move_init->motor_chassis[i].chassis_pid, PID_POSITION, motor_speed_pid, CHASSIS_MAX_OUT, CHASSIS_MAX_IOUT);
		 ...
		 RUDDER_PID_INIT(chassis_move_init,rudder_pid);
		      PID_Init(&chassis_move_init->rudder_speed_pid, PID_POSITION, rudder_speed_pid, CHASSIS_MAX_OUT, CHASSIS_MAX_IOUT);
		      PID_Init(&chassis_move_init->rudder_angle_pid, PID_POSITION, rudder_angle_pid, CHASSIS_MAX_OUT, CHASSIS_MAX_IOUT);
		 //跟随云台PID
...
//		 		 chassis_move.Back_R.ecd_zero_set=7720;
...
//void Power_Charge(fp32 power)
//{
//	if(chassis_move.chassis_power_buffer>60)
//	{
//	power*=2.1f;	
//	}
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    chassis_behaviour_control_set(&vx_set, &vy_set, &angle_set, chassis_move_control);
        ...
        cos_yaw = arm_cos_f32((relative_angle));		
				...
				if(fabs(relative_angle)>0.1f)    //一般情况下不抱圆，起步速度更快
					std_vector=1;
				
        chassis_move_control->vx_set= cos_yaw * vx_set + sin_yaw * vy_set;
        chassis_move_control->vy_set = -1.0f*sin_yaw * vx_set + cos_yaw * vy_set;	
				
//				if(fabs(chassis_move_control->vx_set)>0.1 || fabs(chassis_move_control->vy_set)>0.1 	)				
...
//				Rudder_motor_zero_ecd_set(chassis_move_control);
        ...
        chassis_move_control->vy_set = -1.0f*sin_yaw * vx_set + cos_yaw * vy_set;
			chassis_move_control->chassis_relative_angle_set = rad_format(0.0);
				chassis_move_control->wz_set=20.f;
			if(fabs(chassis_move_control->vx_set_CANsend)>100 || fabs(chassis_move_control->vy_set_CANsend)>100)
			{
//				if(chassis_move_control->chassis_level==1)
//				{
//					chassis_move_control->vx_set *=1.4f;
//					chassis_move_control->vy_set *=1.4f;
//					chassis_move_control->wz_set *=0.5f;
//				}
//				else if(chassis_move_control->chassis_level==2)
//				{
//					chassis_move_control->vx_set *=1.5f;
//					chassis_move_control->vy_set *=1.5f;
//					chassis_move_control->wz_set *=0.6f;
//				}
//				else if(chassis_move_control->chassis_level==3)
//				{
					chassis_move_control->vx_set *=1.6f;
					chassis_move_control->vy_set *=1.6f;
					chassis_move_control->wz_set *=0.7f;
//				}	
      }
			else   //当原地时，加大转速
				{
...
//		if(abs(chassis_motor->ecd_error)<30)
//			ecd_ready=1;
//		else
//			ecd_ready=0;
...
//	fp32 a;
//	a = PID_Calc(&chassis_move.rudder_angle_pid, 0, rudder_motor->ecd_error);
//	rudder_motor->given_current=PID_Calc(&chassis_move.rudder_speed_pid, rudder_motor->, a);
}
...
//	if(ecd_ready==0&&(fabs(chassis_motor->vx)<1.0f)&&(fabs(chassis_motor->vy)<1.0f)&&(fabs(chassis_motor->wz)<0.5f))
//	{
//		for(i=0;i<4;i++)				
//			chassis_motor->motor_chassis[i].give_current=0;	
//	}
//	else
//	{
	for(i=0;i<4;i++)
...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, chassis_move_t *chassis_move_rc_to_vector)
{
    if (chassis_move_rc_to_vector == NULL || vx_set == NULL || vy_set == NULL)
    {
        return;
			...
			if((chassis_move.key_C==8000)&&get_cap.capvot>19)  //根据等级变化开超电后的功率
		{
    kx=1.2f;  
    ky=1.1f;
		kw=1.0f;	
