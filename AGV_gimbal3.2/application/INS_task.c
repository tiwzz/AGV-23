...
fp32 gyro_gyro[3],gyro_angle[3];   //防零漂修改
...
void INS_task(void const *pvParameters)
{
    ...
    while (1)
    {
		...
		gyro_gyro[0]=+bmi088_real_data.gyro[2];
   if(fabs(gyro_gyro[0])>0.09f)
		 gyro_angle[0]+= (gyro_gyro[0]*0.001f);
//	 	gyro_angle[0]=fFilter(gyro_angle[0]); 
	   INS_angle[0]=gyro_angle[0];
//	 if(fabs(gyro_gyro[1])>0.10f)
//		 gyro_angle[1]+= (gyro_gyro[1]*0.001f);
//	   INS_angle[1]=gyro_angle[1];
//	 if(fabs(gyro_gyro[2])>0.10f)
//		 gyro_angle[2]+= (gyro_gyro[2]*0.001f);
//	   INS_angle[2]=gyro_angle[2];
